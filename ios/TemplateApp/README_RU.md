...
> Эталонное развёртывание — **DAN-Gerege SSO** ([sso.gerege.mn](https://sso.gerege.mn)).

## Архитектура

- Приложение → `https://sso.gerege.mn/api/*` (BFF) — с бэкендом напрямую не общается.
...
- Адрес бэкенда: `APIClient.baseURL` (по умолчанию `https://sso.gerege.mn`).
