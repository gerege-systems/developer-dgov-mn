...
import (
	...
	"eidtemplate/pkg/clock"
	"eidtemplate/pkg/logger"
...
type JWTService interface {
	...
	// GenerateOIDCTokenPair нь OIDC token endpoint-ийн (authorization_code →
	// токен) солилцоонд зориулсан access+refresh хос үүсгэнэ. Access токенд
	// audience (хүсэлт гаргасан client_id) болон олгосон scope-ийг шингээж,
	// resource server per-client эрхийг (aud/scope) шалгах боломжтой болгоно.
	// Хэрэв энэ access токеныг refresh хийвэл (одоогийн нийтлэг refresh зам)
	// aud/scope ХАДГАЛАГДАХГҮЙ — RS талын хэрэгжүүлэлт дуусахад refresh замыг
	// ч мөн scope хадгалдаг болгох ёстой.
	GenerateOIDCTokenPair(userID, audience, scope string) (TokenPair, error)
...
type JwtCustomClaim struct {
	...
	Email   string
	Kind    string
	// Scope нь OIDC token endpoint-ээр олгосон зайгаар тусгаарласан scope-ийн
	// жагсаалт (зөвхөн тэр замаар олгогдсон access токенд тавигдана). Audience
	// (хүсэлт гаргасан client_id) нь RegisteredClaims.Audience дотор хадгалагдана.
...
func (j *jwtService) GenerateTokenPair(userID string, isAdmin bool, email string) (TokenPair, error) {
	return j.generatePair(userID, isAdmin, email, "", "")
}

// GenerateOIDCTokenPair нь access токенд audience (client_id) болон scope-ийг
// нэмж хос үүсгэнэ (OIDC token endpoint). isAdmin нь үргэлж false — энэ нь
// downstream RP-д олгосон федерацийн токен, дотоод admin токен биш.
func (j *jwtService) GenerateOIDCTokenPair(userID, audience, scope string) (TokenPair, error) {
	return j.generatePair(userID, false, "", audience, scope)
}

func (j *jwtService) generatePair(userID string, isAdmin bool, email, audience, scope string) (TokenPair, error) {
	access, accessExp, accessJTI, err := j.signAccess(userID, isAdmin, email, audience, scope)
	if err != nil {
...
func (j *jwtService) signAccess(userID string, isAdmin bool, email, audience, scope string) (token string, expiresAt time.Time, jti string, err error) {
	now := j.clock.Now()
	expiresAt = now.Add(time.Hour * time.Duration(j.expired))
	jti = uuid.NewString()
	registered := golangJWT.RegisteredClaims{
		ID:        jti,
	...
	if audience != "" {
		registered.Audience = golangJWT.ClaimStrings{audience}
	}
...
func (j *jwtService) ParseToken(tokenString string) (JwtCustomClaim, error) {
	...
	// Зөвхөн илэрхий access токеныг хүлээн авна. signAccess нь Kind-ийг ҮРГЭЛЖ
	// тамгалдаг тул хоосон/тодорхойгүй Kind нь хүчинтэй токен биш — fail-closed
	// байхын тулд татгалзана (хоосныг access гэж тооцвол refresh эсвэл алдаатай
	// токен access эрхтэй мэт нэвтрэх эрсдэлтэй).
...
func (j *jwtService) parse(tokenString string) (JwtCustomClaim, error) {
	...
	token, err := golangJWT.ParseWithClaims(tokenString, &claims, func(token *golangJWT.Token) (interface{}, error) {
		...
		// Defense-in-depth (declarative): зөвхөн HS256-г зөвшөөрнө (keyfunc-ийн
		// төрлийн шалгалттай давхар), exp-г заавал шаардана, iss-ийг тулгана.
		// Бүх олгосон токен эдгээрийг тогтмол тавьдаг тул хууль ёсны токен унахгүй.
