...
| `ALLOWED_ORIGINS` | `https://template.gerege.mn` | Список разрешённых origin для CORS (через запятую; `*` запрещён) |
...
| `SSO_REDIRECT_URI` | `https://template.gerege.mn/sso/callback` | Должен быть зарегистрирован у клиента SSO **точь-в-точь** |
...
| Переменная | Назначение |
|---|---|
| `OAUTH_ISSUER` | Например, `https://template.gerege.mn`. Провайдер включается **только** при заданном значении |
