...
!!! tip "Язык ответа"
    Фронтенд передаёт язык интерфейса (`mn`/`en`/`zh`/`ru`) в поле `lang`, и
    ассистент отвечает на нём. Если пользователь пишет на другом языке, модель
    следует за пользователем; резервный ответ `degraded` локализуется так же.
