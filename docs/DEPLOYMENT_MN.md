...
```
https://<domain>/wallet/        → wallet API   (nginx /wallet/ prefix-ийг хусна)
...
1. Wallet repo-г clone хийж (`git clone
   ...
   ```env
   COMPOSE_PROJECT_NAME=temp-wallet      # контейнер нэрийн prefix
   WALLET_IMAGE=temp-wallet-gerege-mn    # ← давхцалгүй нэрс: хост дээр өөр
   WALLET_PG_VOLUME=temp-wallet-pg-data  #   wallet stack аль хэдийн байж
   WALLET_NETWORK=temp-wallet_default    #   болзошгүй
   WALLET_ADMIN_IMAGE=temp-wallet-gerege-mn-admin-web
   API_PORT=8094                         # чөлөөт loopback портууд сонго
   ADMIN_API_PORT=8095
   ADMIN_WEB_PORT=3012
   ADMIN_BASE_PATH=/wallet-admin         # Next.js basePath (build-time)
   POSTGRES_PASSWORD=<санамсаргүй> JWT_SECRET=<≥32 санамсаргүй> …
   ```

2. Build, migrate, асаах:
...
4. Эхний эрхүүдийг үүсгэх (нууц бүр зөвхөн **нэг** удаа хэвлэгдэнэ):

   ```bash
   docker compose run --rm --entrypoint /app/client api admin superadmin "Super Admin"
   docker compose run --rm --entrypoint /app/client api create <client_id> "<нэр>"
   ```
