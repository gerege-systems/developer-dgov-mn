...
How to deploy the **Government Developer Portal V3.0** (Цахим засаглалыг бүтээх
...
```bash
git clone https://github.com/gerege-systems/developer-dgov-mn.git /srv/dan
cd /srv/dan
```
...
SSO_NATIVE_CLIENT_ID=developer-dgov-mn-ios   # Hydra client_id for the mobile PKCE flow
...
XYP_CLIENT_ID=<…>
XYP_CLIENT_SECRET=<…>
CORE_API_BASE=https://core.gerege.mn     # user/org find
...
docker logs developer-dgov-mn-migrate-1 | tail -3             # "migration [up] success"
docker logs developer-dgov-mn-hydra-migrate-1 | tail -3       # Hydra schema applied
docker logs developer-dgov-mn-api-1 2>&1 | grep -i error      # should be empty
...
**Government Developer Portal V3.0** — Co-developed by the **Gerege Systems Development Team** and **Claude AI**, 2026.
