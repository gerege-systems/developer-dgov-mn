...
func TestOAuthClientRepository(t *testing.T) {
	...
	t.Run("list returns newest first", func(t *testing.T) {
		second := seedClient()
		second.ClientID = "developer-dgov-mn"
		second.RedirectURIs = []string{"https://developer.dgov.mn/auth/callback"}
		...
		assert.Equal(t, "developer-dgov-mn", list[0].ClientID, "newest client should sort first")
	})

	t.Run("delete", func(t *testing.T) {
		require.NoError(t, repo.Delete(ctx, "developer-dgov-mn"))

		_, err := repo.Get(ctx, "developer-dgov-mn")
		assertErrType(t, err, apperror.ErrTypeNotFound)

		err = repo.Delete(ctx, "developer-dgov-mn")
		require.Error(t, err)
		assertErrType(t, err, apperror.ErrTypeNotFound)
	})
