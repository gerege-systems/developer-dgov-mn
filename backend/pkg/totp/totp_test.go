...
func TestGenerate(t *testing.T) {
	secret, uri, err := totp.Generate("Government Developer Portal V3.0", "bat@dgov.mn")
	...
	assert.Equal(t, "Government Developer Portal V3.0", u.Query().Get("issuer"))
...
func TestValidate(t *testing.T) {
	secret, _, err := totp.Generate("Government Developer Portal V3.0", "bat@dgov.mn")
	require.NoError(t, err)
